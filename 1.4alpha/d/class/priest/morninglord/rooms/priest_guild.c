...
void create() {
  ::create();
  seteuid(getuid());
  set("short", "The Guild of Lathander");
...
int tp, brains;
 ...
 if (TP->query("base_stat/charisma") < 12) {
    write("You are not charismatic enough for us!\n");
    return 0;
  }
 if (!(TP->query_alignment() == "neutral good")){
  ...
  write("The air shimmers, and a rose appears in your hands.\n");
...
set_tp(TP, tp);
 ...
 brains = this_player()->query("stat/intelligence");
  ...
  TP->set("deity", "lathander");
  robes = clone_object(LATHANDER+"obj/rose.c");
  if (!robes) {
