...
nomask varargs int set( string label, mixed value, int access_level ) {
     ...
     label_arr = explode( label, "/" );
        ...
        if( !valid_write( label_arr[0], previous_object() ) ) return 0;
#endif /* SECURE */
	_set( ob_data, label_arr, value );
#ifdef SECURE
...
#endif /* SECURE */
        value = _query( ob_data, explode( label, "/" ) );
#ifdef SECURE
        value = copy( value );
#endif /* SECURE */
...
nomask mixed query_temp( string label ) {
        ...
        if( !valid_read( label, previous_object() ) ) return;
#endif /* SECURE */
        value = _query( tmp_ob_data, explode( label, "/" ) );
#ifdef SECURE
        value = copy( value );
#endif /* SECURE */
        if( stringp( value ) && sscanf( value, "@@%s", func ) ) {
