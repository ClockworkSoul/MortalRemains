...
int tp, brains;
 ...
 if (TP->query("base_stat/strength") < 14) {
  ...
  (!(TP->query_alignment() == "lawful neutral")) &&
  (!(TP->query_alignment() == "lawful evil")) )   {
    write("You are not lawful enough for us!\n");
    return 0;
  }

  write("A voice says, \"Welcome child...\"\n");
  write("The air shimmers, and a suit of full plate armor appears in your hands.\n");
...
set_tp(TP, tp);
 ...
 brains = this_player()->query("stat/intelligence");
  ...
  TP->set("deity", "helm");
  robes = clone_object(HELM+"obj/helmplate.c");
  if (!robes) {
