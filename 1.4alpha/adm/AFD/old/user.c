...
// Chronos added the death sequence and loggging of pkils and kills (7/2/95)
	...
	hit_chance = 30 + str + att_sk + 3*wc - dex - def_sk - 3*ac + hit_mod ;
      ...
      if (!query("vision") || query("blind")) {
        if (query("blind") || !query("infravision")) {
          ...
          if (skill_improve_prob<0) skill_improve_prob = 0;
...
/* Moved this further down in quit() - Ariel
#ifdef LOGOUT_MSG
...
//fix up the alignment.
    if(query_ip_number(killer)) 
         killer->add_alignment((-1*(int)query("alignment")-10)/4);

// A small change in the amt we lose for dying. 1/20th at higher levels, and 
...
string query_short() {
  ...
  return ("A marble statue of " + query("title"));
...
string query_title() {
   ...
   if(!sscanf(str, "%s$N%s", foo,fii))
	...
	if(sscanf(str,"%s$A%s",foo,fii))
		str=substr( str, "$A", query("al_title"));
    (this_object()->query_link())->set("title", str);
	return str;
}
...
int query_level() {
  ...
  if(link) {
     ...
     if (link->query("wizard")) return 21;
