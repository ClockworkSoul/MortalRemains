...
* 00-08-02 Chronos made the mmin and mmout messages wrap
...
varargs int move_player(mixed dest, string message, string dir) {
	...
	if (message == 0 || message == "") {
		...
		message(MSG_MOVEMENT, (string)query_min(dirfrom) + "\n",
         ...
         message(MSG_MOVEMENT, wrap((string)query_mmout()),
		  prev, this_object());
    message(MSG_MOVEMENT, wrap((string)query_mmin()),
		  environment(), this_object());
	    }
	} else {
...
void heart_beat() {
    ...
    // This calls the heart_beat in /std/body/living.c
    ::heart_beat();
