...
// 97-04-42 Cyanide added the cool skull effects and mudwide shout...
// 97-05-02 Zur made it more cpu friendly. (took out like 7 calls to find_living
//    () ).
...
static int confirm_deletion(string str, string name, int banish,
  int delete, int archive) {
	...
	tell_room(environment(ob), 
	  ...
	  subjective((string)ob->query("gender")) + " once stood.\n",
	  ({ ob }) );

	skull=clone_object("/obj/obj/nuke_skull.c");
	skull->set("name", name);

	skull->move(environment(ob));
    } else {
	ob = FINGER_D->restore_body(name);
	if (ob) {
	    DEBUG("Removing "+name+" from gang lists.\n");
	    if (ob->query("gang"))
