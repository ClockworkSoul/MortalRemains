...
static void free_objects()
{
    int val;
#ifdef DEBUG
    DEBUG("Free_object(): Beginning.\n");
#endif

    seteuid(getuid());
    call_out("free_objects", 3600);
    val = efun::reclaim_objects(); 

    // This call_other ensures that the reboot daemon is running.
    // For unknown reasons, it has a tendency to die out after a while...
    // Cyanide - 25 June 1997
    call_other("/adm/daemons/rebootd.c", "???");
#ifdef DEBUG
    DEBUG("Done. "+val+" objects reclaimed.\n");
#endif
}
...
static void crash( string error, object command_giver, object current_object )
{
    log_file( "crashes", mud_name() + " CRASHED on: " + ctime( time() ) +
      " ERROR: " + error + "\n" );
    shout("Game Driver shouts: Oh, fuck. Cyanide's gonna be pissed!!\n");
    //Cyanide   ;)
