...
#define __DEFAULT_BAN ({ })
#define __DEFAULT_INVITE ({ "cyanide" })
...
void create() {
  ...
  set ("c_long", @ENDLONG
You find yourself in a strangely familiar room constructed out of the
...
ENDLONG
    ...
    set("smell", ([
      "default" : "The sweet smell of a fine incense permeates the air.\n",
    ]) );
    set("item_desc", ([
  "mug" : "Chronos's magical 'Endless Double Mocha' Mug (tm).\n",
      ...
      "clock" : "You can't tell what time it is because the clock is constantly running\n"+
                "at different speeds.  Not to mention the fact that it's going backwards..\n",

    ]) );
     call_other("/u/c/chronos/obj/to_do_board", "foo") ;
}
...
int check_prot(object person, string cmd) {
	...
	if (owner) {
        tell_object(owner, "** You sense that "+person->query("cap_name") +
	      " just tried to \"" + cmd +
...
int screen_func (string onoff) {
	...
	case 1 : onoff = "on"; break;
        ...
        tell_object(this_player(), "[Workroom] Invisibility screens are "+
	  "currently "+onoff+".\n");
	return 1;
    } else {
...
void guest_ban (mixed str) {
	...
	banned += ({ str });
        room_log("[BAN]", capitalize((string)str));
    } else {
...
int receive_object (object ob) {
	...
	obname = getuid(ob);
        if ((obname != OWNER) && (obname != "chronos")) {
