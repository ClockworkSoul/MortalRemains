...
// (12-27-96) Cyanide altered the inherited player object so that it calls
//            the new functions query_mass() and query_bulk(), which take
//            into account strength and size. Also revamped max_cap and
//            max_vol.
// (01-03-97) Cyanide fixed the drunk code so that a slightly drunk person
//            doesn't actually fight BETTER. Also converted damage messages
//            to output according to the percentage of hp lost to the
...
varargs int move_player(mixed dest, string message, string dir) {
   ...
   return 0; } 
     ...
     if(!message || message == "") {
	if(dir && dir != "") {
          message("movement", TO->query_mout(dir)+"\n", 
          prev, ({ this_object() }) ) ;
          message("movement", this_object()->query_min(dir)+"\n", 
          environment(this_object()), ({ this_object() }) ) ;
	} else {
          message("movement", this_object()->query_mmout() + "\n", 
          prev, ({ this_object() }) ) ; 
          message("movement", this_object()->query_mmin() +"\n",
          environment(this_object()), ({ this_object() }) ) ;
	}
     } else {
...
//	write ("You are too busy to make an attack!\n") ;
	...
	if(query("hit_points")*100/query("max_hp") <
	   this_player()->query("wimpy")) {
        ...
        // Auto spell casting?
  if (query("autocast") && !query("rest")) command((string)query("autocast")) ;
   if (query("karate")) return ;
...
// Chronos removed the casttype here.. 
   if (query_level() * 3 > random(100)) {
  ...
  hand = (int)query("hand") + 1;
    execute_attack(0,0,hand) ;
  }
  if (query("hasted")) execute_attack(4, 1, hand) ;
  if (query("attacks"))
   for(i=0;i<(int)query("attacks");i++) {
     if (random(100)+1 < query_level()*2)
          execute_attack(0,0,hand) ;
    }
}
...
varargs void execute_attack (int hit_mod, int dam_mod, int hand) {
  ...
  if (!attackers || !attackers[0]) return ;
   if (query("rest")) return ;
  if (!hit_mod) hit_mod = 0 ;
  if (!dam_mod) dam_mod = 0 ;
   switch((int)attackers[0]->query("rest")) {
   case 1: hit_mod += 5; dam_mod += 6; break;
   case 2: hit_mod += 15; dam_mod += 20; break ;
   }
...
// in the attackers queue. If the attackers queue is empty, stop attack call.
    ...
    if (attackers[0]->query("hit_points")<0 || attackers[0]->query("dying")) {
...
// Collect the various statistics needed to get the hit chance and damage.
  ...
  (int)query("stat/dexterity") ;
  dex = attackers[0]->query("stat/agility") +
       attackers[0]->query("stat/dexterity") ;
   ...
   if (hand==1 && !weapon) weapon=query("right_hand") ;
   if (hand==2 && !weapon) weapon=query("left_hand") ;
 ...
 hit_chance = 40 + str + att_sk + 3*wc - def_sk - 3*ac - dex + hit_mod ;
  if (query("drunk")>0) {
   if ( ((int)query("drunk")*3) > ((int)query("stat/spirit")*2) )
    hit_chance + (int)query("stat/spirit")*2 - (int)query("drunk")*3 ;
  }
...
// Attacking invisible creatures is really rather difficult.
  ...
  // Chronos changed this.
   // if ((int)attackers[0]->query("invisible")>0) hit_chance = hit_chance / 5;
  if (!visible(attackers[0], this_object())) hit_chance=hit_chance / 5;
...
// This is the weapon_hit() function being called.
        if (weapon) 
          damage += weapon->weapon_hit(damage, attackers[0]) ;
...
// the appropriate damage messages.
  ...
  damstr="You "+verb1+" "+vname+" with your "+wepstr+
   " and barely nick "+qs+"." ;
   damstr2 = name+" "+verb2+" "+vname+" with "+possessive(this_object()->
    query("gender"))+" "+wepstr+" and barely nicks "+qs+"." ;
   victim = name+" "+verb2+" you with "+TO->query_possessive()+
...
// chronos removed cast types here.
  this_object()->set("capacity", query_capacity());
  this_object()->set("volume", query_volume());
 
   //	Announce the departure of the user.

  message("say", query("cap_name") + " has left the game.\n",
  environment(TO), ({ TO }) ) ;
...
void consistency_check() {
  ...
  delete("magical_offense") ;
  delete("rest") ;
  if ((string)query("race")=="troll") set("time_to_heal", 8) ;
  else set("time_to_heal", 10) ;
  if (!query("al_title")) set("al_title", "neutral") ;
  if (!query("wiz_points") && wizardp(TO)) set("wiz_points", 1, LOCKED) ;
  delete("dying") ;
  switch((string)query("race")) {
    case "drow" : set("resist_type", ([ "magic" : 50,
        "poison" : 200 ]) ); break;
    case "dwarf" : set("resist_type", ([ "magic" : 50 ])); break;
    case "elf" : set("resist_type", ([ "magic" : 50,
        "poison" : 200 ])); break;
    case "half-elf" : set("resist_type", ([ "poison" : 150,
        "magic" : 75 ])); break;
    case "halfling" : set("resist_type", ([ "poison" : 50,
        "blunt" : 150 ]) ); break;
    case "skaven" : set("resist_type", ([ "fire" : 110,
        "cold" : 50, "necromancy" : 120 ]) ); break;
    case "troll" : set("resist_type", ([ "fire" : 300,
        "cold" : 50, "acid" : 200, "electricity" : 50,
        "blunt" : 50 ]) ); break;
    default : break;
  }
}
...
of users, or leave it commented out...
  ...
  if (!this_object()->query("invisible")) {
    ob = present("#GUILD_OBJECT#", this_object());
    if (ob) ob->enter_world();
...
// check_team() ;
   ...
   ob = present("#GUILD_OBJECT#", TO) ;
   if (ob) ob->linkdead(0) ;
   command("band disband") ;
   command("band leave") ;
...
tell_room(environment(), "The linkdead statue of " +
  ...
  "The idle body of " + this_object()->query("cap_name") + 
    " slowly fades from sight.\n", 
   ({ this_object() }));
   }
 
  this_object()->move_player("/d/Start/town/rooms/shop",
...
string query_short() {
  ...
  if (query_rest_status()==1) return 
   query("cap_name")+" is resting here." ;
  if (query_rest_status()==2) return
  query("cap_name")+" is sleeping here." ;
...
int query_invis() { return (int)query("invisible"); }
/* EOF */
