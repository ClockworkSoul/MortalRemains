...
// 960325  - Added backlog for each channel                    -- Perrin@Earthmu
...
// 980117  - Added "to <person> <msg>" support.                -- Chronos@NK
//         - Changed cosmetics and formatting code.
//         - Organized sloppy code.
// 990621  - Filtered out extraneous channels from our I3 
//           network.                                          --Cyanide@MR
...
#define FILTER_EXTRA_CHANNELS 1
...
private static
int admin_channel(string chan,object user,string* add_list,string* rem_list) {
    string ext_ob, err;
...
private static varargs
int create_channel(string chan, object user, string group, int mode,
  object ref ) {
    ...
    if( group ) channels[chan]["priv"]=group;
    if( ref ) channels[chan]["object"]=chan+"#"+base_name(ref)+".c";

    return 1;
}


varargs private static
int kill_user(object user, string chan, int onquit) {
    string ext_ob, err;
    string *usrchanlist;
...
varargs
int delete_channel(string chan, object user) {
    int i;
    object *chuser;
    ...
    for( i=0; i<sizeof(chuser); i++ ) kill_user( chuser[i], chan );
...
parse_channel(string chan, string mesg, string name, int emote)
{
	...
	return log(sprintf("parse_channel: channel %s is not a "+
	    "pointer!  Fixing.", identify(chan)));
    }
    ...
    // Chronos code starts here.
    if (mesg == "to") return tell_user(TP, "["+capitalize(chan)+
	  "] No target for broadcast.");
	...
	mesg ="["+capitalize(chan)+"] "+name+" "+mesg ;
    } else if (mesg[0..2] == "to ") {
	if (sscanf(mesg, "to %s %s", to_name, mesg) == 2) {
	    target = find_player(lower_case(to_name));
	    if (target && visible(target, TP)) to = 1;
	    else {
		tell_object(this_player(),
		  "["+capitalize(chan)+"] Couldn't find anyone named '"+
		  capitalize(to_name)+"'.\n");
		return 1;
	    }
	    if (target == this_player()) {
		tell_object(this_player(),
		  "A little voice inside your head tells you: "+mesg+"\n");
		return 1;
	    }
	} else if (sscanf(mesg, "to %s", to_name) == 1) {
	    CMD_SHOW->cmd_show(chan);
	    return 1;
	}      
    }
	...
	add_chan_backlog(chan,iwrap("["+capitalize(chan)+"] "+name+
	    " [to "+capitalize(to_name)+"]: "+mesg));
    else
	add_chan_backlog(chan,iwrap("["+capitalize(chan)+"] "+name+
	    ": "+mesg));
    while(i--)  {
	if (!objectp(list[i])) continue;
	if (to && list[i]  == target ) {              // The to target.
		...
		tell_object( list[i], "\n" );
	    if (to) {
		message( "channels", iwrap("["+capitalize(chan)+
		    "] "+name+" [to you]: "+mesg), list[i]);
	    }
	} else if (list[i] == this_player()) {  // The speaker.
	    if (emote)
		message("channels",iwrap("["+capitalize(chan)+"] "+mesg),list[i]);
	    else if (to)
		message("channels",iwrap("["+capitalize(chan)+"] "+
		    name+" [to "+capitalize(to_name)+"]: "+mesg), list[i]);
	    else
		message("channels",iwrap("["+capitalize(chan)+"] "+name+": "+
		    mesg),list[i]);
	} else {
	    ignore = (string *) list[i]->query("ignore");
	    if (!pointerp(ignore)) ignore = ({ });
	    if (member_array(lower_case(name), ignore) != -1) continue;
	    if (list[i]->getenv("add_newline")) tell_object(list[i], "\n");
	    if (emote)
		message("channels",iwrap("["+capitalize(chan)+"] "+
		    mesg),list[i]);
	    else if (to)
		message("channels",iwrap("["+capitalize(chan)+"] "+
		    name+" [to "+capitalize(to_name)+"]: "+mesg), list[i]);
	    else message("channels",iwrap("["+capitalize(chan)+"] "+name+
		    ": "+mesg),list[i]);
	    continue;
	}
...
int add_i3channels( mapping chanlist, int bootflag ) {
	...
	// If this works - it will filter out extra channels.
	// Cyanide, 21 Jun 1999

#ifdef FILTER_EXTRA_CHANNELS
	list = query_allowed_channels();
	j = sizeof(list);

	while (j--)
	    if (member_array(i3list[j], list)==-1)
		i3list[j] = "*XXX*";

	i3list = filter_array(i3list, "filter_i3list", TP);
#endif
...
int parse_amcp_channel(string command, string mesg) {
    ...
    TP->receive_message("error", "Unknown channel control message.\n");
    return 1;
...
int filter_i3list( string arg ) { return (arg!="*XXX*"); }
