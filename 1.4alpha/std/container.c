...
// Cyanide redid get_from a bit.  6 Feb 2000
//
// To do:
//      Move get_from and put_into into their own seperate _*.c commands, and
//      update them to accepts moden command formats.
...
int put_into(string str) {
    ...
    //  Check for the form "put 10 credits in bag".

    if (sscanf(str,"%d %s in %s",num,this,that)==3) {
	if(num < 1) {
	    notify_fail ("You must put positive amounts of credits into things.\n") ;
	    return 0 ;
	}
	tht = present(that,TP) ;
	...
	has = TP->query("wealth/"+this) ;
	if (has<num) {
	    notify_fail ("You don't have that many "+this+" credits.\n") ;
	    return 0 ;
	}
	...
	say (TP->query("cap_name")+" puts "+num+" "+this+" "+word+" in "+that+".\n") ;
	return 1 ;
    }
	...
	if (this_object()->query("holding"))  mass = 0; 
	...
	if (environment(this_object())==TP)
	    TP->set("capacity", 
	      (int)TP->query("capacity") - mass);
	write("You put " + word + " in " +tht->query("short")+ ".\n");
	say((string)TP->query("cap_name") + " puts " +
