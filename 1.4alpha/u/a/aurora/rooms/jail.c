...
void init() {
  ...
  call_out("begin_jail", 4, this_player());
}
...
int filter_act(string str) {
    if(query_verb() == "says" || query_verb() == "tell" || query_verb() == "shout") {
        ...
        tell_room("/wizards/aurora/workroom.c", "You hear a haunting wail.\n");
    }
    else {
        write("Hey, you are here for a reason, prolly cuz you were being very annoying.  So you can just sit and stew for a bit.\n");
    }
    return 1;
}

void begin_jail(object tp) {
  string nom;

  nom = (string)tp->query_name();
  visitors[nom] = 1;
  tell_object(tp, 
    "You are suddenly scooped up and tossed into a jail cell.\n");
  call_out("continue_jail", 5, tp);
}

void continue_jail(object tp) {
  string nom;
  int beat, deaths;
