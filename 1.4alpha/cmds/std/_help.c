...
// Emeradii@EarthMud: Jan 3rd, 1995:
...
// Cyanide: Jul 19th, 1999: Made bad helps log to "bad_help" as a QC measure.
...
{
    return ( item[0..8] == "/doc/help" ||
      item[0..10] == "/doc/skills" );
}
...
call_help(string path, string topic)
{
	...
	case SKILLS_DIR : group = "Skills Help";
	    temp = get_banner(group, topic);
	    this_player()->more(explode( (temp+read_file(path+"/"+topic)+"\n"), "\n" ), 0, 1);
	    return 1;
	...
	if (!ob) 
	{
	    message("error", temp + "A command with that name exists, but it "+
	      "has an error in it.\n", this_player());
	    return 1;
	}
...
cmd_help(string topic)
{
    ...
    DEBUG("CMD_HELP: "+TPN+" attempting to access: "+topic+"\n");
	...
	case "commands"    : search_path = explode((string)act_ob->query("PATH"), ":");
	    ...
	    log_file(BAD_HELP, "[ "+extract(ctime(time()),4,15)+" - "+TPN+" ]\t\t"+topic+"\n");
	    return 1 ;
	...
	if (!sizeof(list))
	{
	    log_file(BAD_HELP, "[ "+extract(ctime(time()),4,15)+" - "+TPN+" ]\t\t"+
	      topic+"\n");
