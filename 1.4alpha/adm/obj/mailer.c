...
// Cyanide, 990125  Added support for .signature or .sig files.
...
get_text() {
    ...
    // Cyanide adds support for .signature files here.  25 Jan 99
    tmp1 = user_path(owner) + ".signature";
    if (file_size(tmp1) == 0) tmp1 = user_path(owner) + ".sig";
    if (file_size(tmp1) >= 0) 
	newmsg["message"] += ("\n-\n" + read_file(tmp1));
...
{
    /* On 1 Jun 99 this started acting up for no good reason.
       Cyanide saw no reason not to remove it...
