...
int cmd_chfn( string arg ) {
    ...
    write("Changing finger information on " +
      capitalize((string)link->query("name"))+" at " +mud_name() + ".\n") ;
...
static void new_email(string e) {
    ...
    write("Email publicly visible? ["+
      (TP->query("email_visible") ? "Y" : "N")+"] : ");
    input_to("email_vis");
}

static void email_vis(string yorn) {
    if ( yorn && strlen( yorn ) > 0 ) { 
	yorn = yorn[0..0];
	if (lower_case(yorn)=="y")
	    TP->set("email_visible", 1, OWNER_ONLY);
	else
	    TP->set("email_visible", 0, OWNER_ONLY);
    }
}
