...
// Barfboy@Sodden Earth hacked this into the wearable container. 6-5-97
// Cattt@Mortal Remains took and distributed June, 99
...
void unequip(int silent){
    object fellow, obj;
    string type, tmp, short;
    mapping armor;
    int Class;

    short = this_object()->query("short");
    fellow = environment(this_object());
    if(fellow){
	obj = this_object();
	type = obj->query("type");
	...
	if(!silent)
	    tell_object(fellow, "You remove "+short+".\n");

	if(!fellow && environment(fellow) && !silent)
	    tell_room(environment(fellow),fellow->query("cap_name")+" removes "+short+".\n", ({ fellow }));
