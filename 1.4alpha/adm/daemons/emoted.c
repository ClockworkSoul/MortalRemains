...
/*
   ...
   Cyanide got rid of the ugly * on 4 Apr 98.
   Cyanide made it respect invis on 30 May 99.
*/
  ...
  object target, int test, int same_super)
{
	...
	if (target && !same_super) {
	    result = "From afar, " + result;
	    result = replace_string(result, "afar, You", "afar, you");
	}
	if (test) {
	    write("me:\n" + result + "\n");
	} else {
...
parse(string command, string rest, int test)
{
	...
	if (mapp(tentry)) {
	    ...
	    if (target && !visible(target, TP)) target = 0;
