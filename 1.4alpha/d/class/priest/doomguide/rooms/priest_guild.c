...
void create() {
  ::create();
  seteuid(getuid());
  set("short", "The Guild of Kelemvor");
...
int tp, brains;
 ...
 if (TP->query("base_stat/constitution") < 12) {
  ...
  write("The air shimmers, and a silver headband appears in your hands.\n");
...
TP->set_skill("Undead Lore", 2);
 ...
 brains = this_player()->query("stat/intelligence");
  ...
  TP->set("deity", "kelemvor");
  robes = clone_object(KELEMVOR+"obj/headband.c");
  if (!robes) {
