...
// 08 May 97  Cyanide added query_thac0, which in turn calls upon the
//            property daemon he created about 2 days ago. Saves mem.
...
varargs void init_setup(int reconnect) {
    ...
    set("max_cap", "@@query_max_cap");
    set("capacity", query("max_cap"));
    set ("capacity", 5000);
...
delete("regeneration");
    ...
    if (this_player()->query("base_resist")) 
	set("resist_type", this_player()->query("base_resist") );
	...
	if (this_object()->query_level("wizard")){
	    ...
	    set("magic_resist", (query_level())*5 );
	    if ((int)query("magic_resist") > 95) set ("magic_resist", 95);
	}
	set("xp_penalty", 10);
	break;
    case "drow" : 
	set("magic_resist", (query_level())*5 );
	if (query("magic_resist") > 85) set ("magic_resist", 85);
	set("xp_penalty", 15);
	break;
    case "aasimon" : 
	if (this_object()->query_level("wizard")){
	...
	break;
    default : set ("magic_resist", 0); break;
    }

    if (query("base_mr")) set("magic_resist",(query("base_mr")));
if (query("faction/Faction") == "Godsmen") add("xp_penalty", -10);
	...
	//and load signature items (ilz)
	if (this_object()->query("signature_item")){
	    ob = clone_object(this_player()->query("signature_item"));
	    if (ob) {
		ob->set("short", ((query("cap_name"))+"'s "+(ob->query("short"))) );
		ob->set("prevent_sell", 1);
		ob->set("prevent_drop", 1);
		ob->move(this_object());
	    } else {
		tell_object(TO, "Error in loading you signiture item. Notify admin!\n");
	    }
	}
    }
...
void receive_message(string Class, string msg) {
	...
	if((int)this_player()->query("invisible") == 2 &&
	  !adminp(geteuid(this_object())))
	    return;
	if((int)this_player()->query("invisible") == 3 &&
	  !(member_group(geteuid(this_object()), "root")))
	    return;

	if((int)this_player()->query("invisible") == 4)
	    return;
    }
...
int query_thac0() {
    return PROPERTY_D->query_THAC0(this_object());
}

string query_possessive() {
    return possessive((string)query("gender"));
}
