...
#undef CAP_NAME
#undef LINK_CAP_NAME
#define CAP_NAME capitalize(this_object()->query("name"))
#define LINK_CAP_NAME capitalize(link_data("name"))
#endif

#define HALL "/d/Fooland/hall" // Leto
#define ORIGIN_DRIVER 1 // Leto

static varargs void complete_setup (string str) ;
static void die();
static int create_ghost();
int coins_carried();
//int query_permission(string prop); Leto found this obsolete? prototype
void init_setup();
...
void consistency_check() {
	...
	if (query("special_say")=="shout") delete ("special_say") ;
#ifdef NO_PKILL
    if (link_data("last_on") < 783090000)
        set("no_attack", 1, READ_ONLY);
#endif
// This little check added by Blue, 941022.  The time is
// the current time (when I added it) plus 15 minutes.  We
// don't want to change back the settings of people who
// have deliberately removed no_attack after this time.
}
...
void setup() {
   ...
   // Set termtype, info is in connection.c
    ...
    set("termtype",this_object()->query_link()->query("termtype"));
  ...
  write("Terminal type is "+query("termtype")+".\n\n");
...
// I think we need this. There is a Big Problem.
...
void hide(int i) {
  ...
  mapping query_error() {
     ...
     if (caller[0..13] == CMD_DBXBASE) return last_error;
       else write("No permission to query error!");
  }
...
// This is only called when the driver is compiled with 
// RECEIVE_SNOOP enabled.
// It is the place to add chars like % or colour, or strip
// characters which might mess up your terminal.

nomask void receive_snoop(string arg) {

replace_string(arg, ESC, " "); // This disables the escape code.
arg = "Snoop>"+arg;
receive(arg);
}
