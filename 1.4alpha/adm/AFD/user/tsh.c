...
// kills history completely..... Leto 942711
   ...
   this_object()->force_me("SILENTtune chat in");
  if (wizardp(this_object())) this_object()->force_me("SILENTtune wiz in");
   if (member_group((string)this_object()->query("name"),"admin"))
   this_object()->force_me("SILENTtune admin in");
  if (member_group((string)this_object()->query("name"), "arches"))
    this_object()->force_me("SILENTtune arch in");
} // initialize_tsh
...
nomask string process_input(string arg)
{
   ...
   if( strlen( arg ) > 5 && arg[0..5] == "passwd" ) {
      printf( "process_input() override of passwd.\n" );
