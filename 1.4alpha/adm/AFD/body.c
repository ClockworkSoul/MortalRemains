#define MSG(xx) tell_object(find_player("chronos"), xx + "\n");
...
/*
// Chronos doesn't think we even use it.. why put the code in every instance?
...
// and all soul commands, and most command messages.
  ...
  // shout blocking.
  if (query("shout_block") && class == "shout") return ;

  // rest status.. filter out stuff they won't be seeing.
  if ((int)this_object()->query("rest")==2) {
    switch(class) {
     case "movement" :
     case "emote" :
     case "combat" :
...
void net_dead() {
  ...
  message("linkdeath", (string)this_object()->query("cap_name") + 
  " suddenly turns into a solid marble statue.\n", environment(),
    ({ this_object() }) );
  MSG("check");
...
int query_rest_status() {  return (int)this_object()->query("rest"); }
