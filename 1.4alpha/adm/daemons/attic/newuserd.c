...
int query_valid_stat(string race, string stat, int val) {
    ...
    if (val < minarray[i]) return -2;
    if (val > maxarray[i]) return -1;
...
You have the following options :
   1 - Reroll your stats
   2 - Interchange two stats
   3 - Change your handedness
   4 - Choose a new gender
   5 - Choose a new race
   6 - Choose a new alignment
   7 - Done
END_MENU
    );
...
static void menu_choice (string choice, object user, object body) {
    ...
    case "7" : finish_up(user, body); break;
...
static void get_stat_switch3 (string choice, int statA, object user, object body) {
    ...
    val = query_valid_stat(body->query("race"), statName, val);
    if (val < 1) {
	clrscr(user);
	switch(val) {
	case -1 : tell_object(user, val+" is above the maximum value for "+
	      statName+".");
	    break;
	case -2 : tell_object(user, val+" is below the minumum value for "+
	      statName+".");
	    break;
    ...
    val = query_valid_stat((string)body->query("race"), statName, val);
    if (val < 1) {
	clrscr(user);
	switch(val) {
	case -1 : tell_object(user, val+" is above the maximum value for "+
