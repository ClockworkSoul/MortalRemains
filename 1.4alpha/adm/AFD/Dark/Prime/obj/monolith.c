...
#undef IVORY	
	...
	// various defines.
#define BBOARD_HELP     "/d/Dark/Prime/obj/monolith.help"
...
#define MAX_MESG_minus_1   29     // Don't want to compare int vs string
#define MIN_MESG	10	// min # notes to leave on board
#define MIN_MESG_plus_1    11 // For archive kludge, Leto
...
create()
{
    ...
    set_default_long_text();
    set("id",({"monolith","board","bulletin board"}));
    set("silent_look", 1);
    messages = ({ });
    num_messages = 0;
    id_ref = 0;
...
followup_note(string str)
{
 ...
 if ( str == MAX_MESG_minus_1 ) {
   archive_board();
  // str = (string)MIN_MESG_plus_1; // Use this for v20.* MudOS
}
