// (06-06-96) Chronos revamped to work with the new lib.
// container.c
...
//  Check for the form "put 10 coins/coin in bag".
...
// Is the container open?
    ...
    res = ob->move(tht) ;
    if (res!=MOVE_OK) {
      notify_fail ("There isn't enough room.\n") ;
     TP->credit(num,1) ;
      ob->remove() ;
      return 0 ;
    }
    if (num==1) word= "coin" ; else word="coins" ;
    write ("You put "+num+" "+word+" in "+that+".\n") ;
    say (TPN+" puts "+num+" "+word+" in "+that+".\n") ;
  log_file("MONEY_XFER", "["+ctime(time())+"] "+TPN+
  " put "+num+" "+word+" in "+that+".\n  --> "+file_name(this_object())+
  "\n") ;
    return 1 ;
  }
...
//  Check for the stand "put thingy in bag" format.
  ...
  if (tht != this_object()) return 0 ;
   ...
   if (!inv) {
    ...
    for(num=0;num<sizeof(inv);num++) {
  if (inv[num]->query("prevent_insert") || inv[num]->query("prevent_drop"))
      continue ;
    mass = inv[num]->query("mass") ;
    word = inv[num]->query("short") ;
    res = (int)inv[num]->move(tht) ;
   if (res == MOVE_OK) {
   ...
   if (environment(TO)==TP)
    ...
    say(TPN+" puts "+word+" in "+tht->query("short")+".\n") ;
      continue ;
  } 
  }
  write("Done.\n") ;
  ...
  if(tht != TO)  return 0;// Keep on looking ...
...
// If the player is carrying the container, his capacity has to go down.
  ...
  if (res == MOVE_NO_ROOM) write("There isn't enough room left to put that in.\n");
  else if(res == MOVE_TOO_HEAVY) write("It is too heavy.\n");
  else if (res == MOVE_NOT_ALLOWED) write("You can't do that.\n");
  return 1; 
}
...
int get_from(string str) {
  ...
  if (!str) {
  write("Get what from what?\n");
   return 1;
  }
 
//  Check to see if the user can see what they are doin.
...
// Check for the form "get 20 coins/coin from sack".
...
// Is it closed?
    ...
    if (!ths) {
      notify_fail ("There aren't any coins in "+that+".\n") ;
      return 0 ;
    }
    if (ths->query_number()<num) {
      notify_fail ("There aren't that many coins in "+that+".\n") ;
      return 0 ;
    }
    ...
    say (TPN+" gets "+num+" "+word+" from "+that+".\n") ;
  log_file("MONEY_XFER", "["+ctime(time())+"] "+TPN+" got "+
   num+" "+word+" from "+that+".\n  --> "+file_name(this_object())+"\n") ;
    return 1 ;
  }
...
// If we got here, it means we're doing a "get all from container".
    ...
    if (sizeof(contents) > 0 ) {
      for (i=0;i<sizeof(contents);i++) {
        ths = contents[i];
        if((int)ths->query("prevent_get")) continue;
        if (!ths->query("short")) continue ;
...
// Change the mass of the container.
          ...
          say(TPN+" gets "+word+" from "+tht->query("short")+".\n") ;
        }
           if (res == MOVE_NO_ROOM) {
              write("Can't carry "+ths->query("short")+".\n");
              if (environment(TO)==TP) TP->set("capacity", (int)TP->query("capacity")+mass) ;
            }
          }
          return 1;
        }
  ...
  return 1;
      ...
      write(capitalize(this) + " is not here.\n");
     return 1;
    }
    write(capitalize(that) + " is closed.\n");
    return 1;
  }
  return 0;
}
...
string query_long(string str) {
  ...
  int i;
  mixed desc;
  if (TO->receive_objects()) desc = open_long_desc ; else desc = closed_long_desc ;
  if ((int)query("lock")>0) desc = desc + "It is locked.\n" ;
...
int open_container(string str) {
 //Check to see if the user can actually see anything.
  ...
  if (stringp(str) && TO->id(str)) {
    if (TO->receive_objects()) write("It is already open.\n");
    else {
      if ((int)query("lock")>0) {
        write ("It's locked!\n") ;
        return 1 ;
      ...
      TO->toggle_closed();
      write("You open "+TO->query("short")+".\n");
      say(TPN+ " opens "+TO->query("short")+ ".\n");
    }
    return 1;
  }
  return 0;
...
//Check to see if the user can see what they are doing.
  ...
  if (stringp(str) && TO->id(str)) {
    if (!TO->receive_objects()) write("It is already closed.\n");
    else if (!TO->toggle_closed()) write("It cannot be closed.\n");
    else {
      write("You close "+TO->query("short")+".\n");
      say(TPN+" closes "+TO->query("short")+".\n");
    }
    return 1;
  }
  return 0;
