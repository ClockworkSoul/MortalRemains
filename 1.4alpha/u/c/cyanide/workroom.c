/*
**  FILE:  workroom.c  (private - Version 4.1)
...
#define __DEFAULT_INVITE ({ "chronos", "spooner" })
#define SAVE_FILE user_path(getuid())+"data/workroom"
...
"Stop looking at my mug",
"Led Zeppelin!",
"This mug has been pilfered from the SUNY Stony Brook Computer Labs",
"BNL Phenix Project",
"R.H.I.C.",
...
void create() {
    ...
    set ("long", "@@query_long");
    set ("exits", ([
      "west" : "/u/c/cyanide/palace/waitingroom",
      "school" : "/d/Prime/School/room/entrance.c",
      "start" : START,
      "void" : VOID,
    ]) );
    set ("c_long", wrap(
	...
	"cabinet in the corner. To the west is the office door.") );

    set("item_desc", ([
      "desk" : wrap("It is quite old, and solid mohogany. It is "+
	"in mint condition.\nThere is a mug on it."),
      "mug" : wrap("It reads: \"" +
	mugArray[ random(sizeof(mugArray)) ] +
	".\""),
      "window" : wrap("It offers a breathtaking view of "+
	MUD_NAME+"."),    
      "cabinet" : "It is very sturdy and magically locked.\n",
      MUD_NAME : "What, are you expecting some kind of secret?\n",
    ]) );

    call_other("/u/c/cyanide/obj/to_do_board", "foo");

    /*
	dragon = clone_object("/u/c/cyanide/toys/loretta.c");
	dragon->move(TO);
    */
}
...
void init() {
    ...
    add_action ("vis_check", "invis");
    add_action ("summ_func", "summon");
    add_action ("summ_func", "asummon");
    add_action ("summ_func", "su");
    //These least three are so you don't have to invite to summon. :)
}

string query_mug() {
    return mugArray[ random(sizeof(mugArray)) ]; 
}

int query_shields() {
    if (getuid(previous_object())!=OWNER) return 0;

    return shields;
}

int query_invited(string str) {
    if (getuid(previous_object())!=OWNER) return 0;

    str = lower_case(str);
    return (str=="cyanide" ||
      member_array(str, invited) + 1
    );
}

int query_banned(string str) {
    if (getuid(previous_object())!=OWNER) return 0;

    str = lower_case(str);
    return (str != "cyanide" &&
      member_array(str, banned) + 1
    );
}
...
int receive_object (object ob) {
	...
	obname = getuid(ob);
	if ((obname != OWNER) && (obname != "cyanide")) {
...
int invis_check( string str ) {
    string obname = TP->query("name");
    if (str=="none" || !screen) return 0;

    if ((obname != OWNER) && (obname != "cyanide")) {
	write("It seens that you cannot do that here.\n");
	return 1;
    }
...
void guest_help() {
    ...
    write(@EndHelp
        Private Workroom V4.0 - by Cyanide
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
varargs int room_log (string type, string name, string cmd) {
    ...
    if (getuid(this_player()) != (string)OWNER) return;
